...
function emptyRunStats(){return{
 kills:0,impacts:0,friendlyFire:0,manualShots:0,manualKills:0,
 weaponShots:emptyWeaponMap(),weaponKills:emptyWeaponMap()
}}
let runStats=emptyRunStats();
function defaultRecords(){return{bestScore:0,bestLevel:1,totalKills:0,games:0,bestRun:null,lastRun:null}}
...
function getRecords(){
 try{
  ...
  return Object.assign(defaultRecords(),raw);
...
function updateDecoys(now){
 [...decoys].forEach(d=>{
  ...
  const oldR=Math.hypot(d.x-50,d.y-50);d.x+=d.vx*dt*100;d.y+=d.vy*dt*100;const rr=Math.hypot(d.x-50,d.y-50);
  DECOY_CIRCLES.forEach((cr,idx)=>{
   if(oldR<cr&&rr>=cr&&!d.crossed.has(idx)){
    d.crossed.add(idx);const base=Math.atan2(d.vy,d.vx),speed=Math.hypot(d.vx,d.vy),a=base+(Math.random()<.5?-1:1)*(.22+Math.random()*.18);
    addDecoy(d.x,d.y,Math.cos(a)*speed,Math.sin(a)*speed,[...d.crossed]);tone(940,.025,.008,"triangle");
   }
  });
...
function startGameCommon(){
 ...
 ctx();gScore=0;gLevel=1;gLives=3;kills=0;spawned=0;sessionKills=0;runStats=emptyRunStats();
...
function goHome(){if(running&&!trainingMode)commitRecords();running=false;clearSavedGame();clearTimeout(spawnTimer);cancelAnimationFrame(raf);clearObjects();clearMines();clearDecoys();if($("#over").open)$("#over").close();$("#game").hidden=true;$("#home").hidden=false;$("#freeSettings").hidden=true;trainingMode=false;document.body.classList.remove("training-mode");$("#modeBadge").hidden=true}
